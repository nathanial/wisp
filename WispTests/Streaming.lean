...
test "Stream response body" := do
  ...
  let stream ← assertOk task.get "stream bytes"
...
test "Stream with forEachChunk" := do
  ...
  let task ← client.executeStreaming req
  let stream ← assertOk task.get "stream chunks"
...
test "Streaming status check helpers" := do
  ...
  let task ← client.executeStreaming req
  let stream ← assertOk task.get "stream 201"
...
test "Streaming 404 response" := do
  ...
  let task ← client.executeStreaming req
  let stream ← assertOk task.get "stream 404"
  let _ ← stream.readAllBody
  stream.status ≡ 404
...
test "Streaming headers accessible" := do
  ...
  let task ← client.executeStreaming req
  let stream ← assertOk task.get "stream headers"
  let _ ← stream.readAllBody
...
test "Streaming read body as text" := do
  ...
  let task ← client.executeStreaming req
  let stream ← assertOk task.get "stream text"
